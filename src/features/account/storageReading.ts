import type { StorageUsageDto } from "@/api/generated/rekordoAPI.schemas";
...
export function readStorage(usage: StorageUsageDto): StorageReading {
  const used = usage.usedBytes ?? 0;
  const quota = usage.quotaBytes ?? 0;
  const photos = usage.photoCount ?? 0;
