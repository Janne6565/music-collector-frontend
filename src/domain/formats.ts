 ...
 * is what `copyFormat` answers when neither the copy nor the catalogue has said — a
 * sentinel that has to exist, because every screen draws, filters and counts a format and
 * a nullable one would put the same guard in a dozen places.
 *
 ...
 * has said what it is yet, and any click fixes that. Mirrors `CHOOSABLE_FORMATS` in
 * rekordo-mobile/src/domain/formats.ts.
 */
