# Design prompt — turn 11: motion and polish

Paste into Claude Design, project `a1b6280a-eae4-4ab0-aab3-68ea4a303c9b`
(`Music Collector.dc.html`). Written 2026-08-25, after turn 10.

---

This is **turn 11 of Music Collector: motion and polish**. Nothing in it should move a
box. Every screen through turn 10 is built and shipped and I am happy with the layouts —
...
Every item detail screen (1g) recolours its content region to the dominant colour of the
sleeve, light or dark; the sidebar deliberately stays paper.
...
## The one piece of async that is visible on a finished screen
...
is why opening a record took seconds. It now happens *beside* the screen: the record
opens instantly in neutral paper chrome, and the sleeve's colour lands 0.1–4 s later —
sometimes recolouring the whole content region from paper to near-black.

**Please design that recolour explicitly.** It is the one moment in the app where the
screen changes under the reader without being touched. I need to know how long it takes,
what eases, whether text and the accent colour cross-fade with the background or lag it,
and what it does when the answer is "no theme" and nothing should change at all. A hard
...
1. **Library (1f) → item detail (1g) and back.** A full route swap today. The cover the
...
2. **Add sheet (6a) and copy details sheet (8d).** Native `<dialog>` over a dimmed
   library. Appears and disappears instantly, with no backdrop fade and no exit at all.
   Both enter and exit, please, plus what Escape/backdrop-dismiss looks like.
...
5. **Editing.** The detail screen's editor unfolds in place. Today it is a hard
   show/hide.
6. **Deleting a copy.** Row vanishes, route returns to the library, grid reflows. Three
   uncoordinated things at once.
7. **Empty and first-run states.** A collection of zero records, an empty wishlist, a
   search with no results.

**Mobile (Expo)**

8. **Tab changes**, and **push to a record** and back.
9. **The add flow (10a) and the artist screen (10c)** — search results replacing under a
   debounced field, and a pressing being added from a discography.
10. **The photo strip** — a sleeve photo added from the camera or library.
